		...
		#for key in f1_dict.keys():
			...
			if key.split("_")[0] == "superscaffold54":
				if int(key.split("_")[1]) > 16379243: # Removing the non-Z linked part of superscaffold54
					...
					print(key.split("_")[0]+"\t"+key.split("_")[1]+"\t"+key.split("_")[2]+"\t"+str(pi_count_key/int(element[30])))#+"\t"+str(int(element[14])/int(element[13]))+"\t"+str((int(element[16])-int(element[15]))/int(element[13]))+"\t"+str(int(element[19])/int(element[13])))  # repeat density# CDS density
			else:
			#print(element[11])
				print(key.split("_")[0]+"\t"+key.split("_")[1]+"\t"+key.split("_")[2]+"\t"+str(pi_count_key/int(element[30])))#+"\t"+str(int(element[14])/int(element[13]))+"\t"+str((int(element[16])-int(element[15]))/int(element[13]))+"\t"+str(int(element[19])/int(element[13])))  # repeat density# CDS density
